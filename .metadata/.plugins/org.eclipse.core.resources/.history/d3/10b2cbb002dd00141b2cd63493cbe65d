...
//		return dt;
	...
	//TODO dit fixen want nadenken gaat echt ni meer #ohgodwhy
	protected boolean isUnderWater(double xPos, double yPos) {
		int[][] tiles = this.getTiles(xPos,yPos);
		for (int[] tile: tiles) {
			try {
				if (this.getWorld().getGeologicalFeature(pixelX, pixelY) == 2) {
					return true;
				}
			} catch (IllegalPixelException e) {
				System.out.println("oei twerkt niet");
	...
	protected boolean isInMagma(double xPos, double yPos) {
		...
		for (int[] tile: tiles) {
			try {
				if (this.getWorld().getGeologicalFeature(pixelX, pixelY) == 3) {
					return true;
				}
			} catch (IllegalPixelException e) {
				System.out.println("oei twerkt niet");
