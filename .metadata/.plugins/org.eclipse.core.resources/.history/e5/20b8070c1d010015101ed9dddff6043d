...
public class StatementTest {
	...
	@Test
	public void StopRunTestLeft() {
		ParseOutcome<?> outcome = facade.parse("object o; stop_run left;");
		Program program = (Program) outcome.getResult();
		Buzam buzam = facade.createBuzamWithProgram(20, 499, sprites, program);
		facade.addBuzam(world, buzam);
		buzam.startMoveLeft();
	...
	public void breakStatementTest() {
		ParseOutcome<?> outcome = facade.parse("while true do start_jump; break; done stop_jump;");
		Program program = (Program) outcome.getResult();
		Buzam buzam = facade.createBuzamWithProgram(20, 499, sprites, program);
		facade.addBuzam(world, buzam);
		// evaluating the loop condition takes 0.001 seconds and start_jump consumes
		// another 0.001 seconds
		program.execute(0.002);	
		assertTrue(buzam.isJumping());
		// break is invoced so the while loop ends, then stop_jump is invoced
		program.execute(0.002);	
		assertFalse(buzam.isJumping());
	}
